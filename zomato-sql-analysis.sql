...
-- Q2) how many dates does the customers visited zomato?
...
select userid , first_pur , product_id , gold_signup_date from 
(select c.userid , c.first_pur , c.product_id , c.gold_signup_date , rank() over(partition by userid order by c.first_pur) as rnk from
(select a.userid, a.created_date as first_pur ,a.product_id ,b.gold_signup_date
from sales a inner join goldusers_signup b on a.userid = b.userid
and created_date > gold_signup_date)c )d where rnk = 1;

-- Q6) WHICH ITEM WAS PURCHASED JUST BEFORE BECAMING THE GOLD MEMBER
...
-- Q7) WHAT IS THE TOTAL ORDERS AND AMOUNT SPENT FOR EACH MEMBER BEFORE THEY BECAME A GOLD MEMBER
...
-- Q8)IF BUYING EACH PRODUCT GENERATES POINTS.. FOR EX 5RS = 2 ZOMATO POINTS AND EACH PRODUCT HAS DIFFERNT PURCHASING
-- POINTS FOR EX FOR P1 5RS = 1ZOMATO POINT , FOR P2 10RS = 5 ZOMATO POINT AND P3 5RS = 1 ZOMATO POINT

-- CALCULATE POINTS COLLECTED BY EACH CUSTOMERS AND FOR WHICH PRODUCT MOST POINTS HAVE BEEN GIVEN TILL NOW

select userid ,sum(total_points) as total_earned_points, sum(total_points)*2.5 total_money_earned from
...
-- Q9) In the first one years after a customer joins the gold progess (including their join date) irrespective
...
SELECT a.userid, a.created_date, a.product_id, b.gold_signup_date, d.price, round(d.price * 0.54) AS total_points_earne FROM sales a
JOIN goldusers_signup b ON a.userid = b.userid JOIN product d ON a.product_id = d.product_id
WHERE a.created_date BETWEEN b.gold_signup_date AND DATE_ADD(b.gold_signup_date, INTERVAL 1 YEAR);
      
      
-- Q10) rnk all the transaction of the customers
select * , rank() over(partition by userid order by created_date) rnk from sales;

-- Q11) Find all the transactions for each member whenever they Are a zomato gold member transaction mark as n/a
